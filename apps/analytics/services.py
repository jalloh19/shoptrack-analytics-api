...
class AnalyticsService:
    ...
    def get_user_behavior_analytics(user_id):
        ...
        # Product affinity
        product_interactions = user_events.filter(
            event_type__in=['added', 'purchased']
        ...
        return {
            ...
            'purchase_rate': round((purchased_carts / total_carts * 100), 2) if total_carts else 0,
            'abandonment_rate': round((abandoned_carts / total_carts * 100), 2) if total_carts else 0,
    ...
    def get_product_insights(product_id):
        ...
        event_counts = product_events.values('event_type').annotate(
            count=Count('id')
        )
    ...
    def get_time_based_metrics(days=30):
        ...
        for cart in recent_carts:
            ...
            if events.count() > 1:
                duration = (events.last().timestamp - events.first().timestamp).total_seconds()
                cart_sessions.append(duration)
        
        avg_session_duration = sum(cart_sessions) / len(cart_sessions) if cart_sessions else 0
        
        # Event frequency
        events_by_day = recent_events.extra({
            'date': "DATE(timestamp)"
    ...
    def _get_most_active_hour(events_queryset):
        ...
        hour_activity = events_queryset.annotate(
